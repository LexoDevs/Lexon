...
void VulkanTexture::destroyImageTexture(){
    vkDestroyImage(m_Context.device, m_Context.textureImage, nullptr);
    vkFreeMemory(m_Context.device, m_Context.textureImageMemory, nullptr);


};

void VulkanTexture::createTextureImageView(VkImageAspectFlags aspectflags){
        m_Context.textureImageView = createImageView(m_Context.textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
...
VkImageView BufferI::createImageView(VkImage image, VkFormat format,  VkImageAspectFlags aspectFlags) {
...
void VulkanTexture::createImageViews(VkImageAspectFlags aspectflags){
    ...
    for (uint32_t i = 0; i < m_Context.swapchainImages.size(); i++) {
         m_Context.swapChainImageViews[i] = createImageView(m_Context.swapchainImages[i], m_Context.swapChainImageFormat, aspectflags);
    }
};
