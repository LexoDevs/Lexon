...
void VertexBuffer::updateUniformBuffer(Swapchain swapchain, uint32_t currentImage, ObjectInstance mesh, CameraView camera) {
    ...
    float aspectratio = swapchain.getExtentSwapchain().width / (float) swapchain.getExtentSwapchain().height;

    mesh.SetMatrixModel(ubo, time, glm::vec3(0.0f, 0.0f, 0.0f));
    camera.SetCameraView(ubo, aspectratio, time, glm::vec3(2.0f, 2.0f, 2.0f));

    //mesh.SetMatrixModel(ubo[1], time ,glm::vec3(1.0f, 1.0f, 5.0f));
    //camera.SetCameraView(ubo[1], aspectratio, time, glm::vec3(2.0f, 2.0f, 2.0f));

    //UniformBufferObject ubo = {ubo[0], ubo[1]}

    memcpy(uniformBuffersMapped[currentImage], &ubo, sizeof(ubo));
        //memcpy(uniformBuffersMapped[currentImage], &ubo[1], sizeof(ubo[1]));
